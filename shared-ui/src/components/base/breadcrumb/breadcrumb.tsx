...
const BreadcrumbLink = memo(
  ({ item, isHome = false }: { item: BreadcrumbItem; isHome?: boolean }) => {
    const baseClasses =
    ...
    return (
      <AriaLink href={item.href} className={baseClasses}>
        {isHome ? (
          <Home03
            className='size-5 transition-inherit-all text-gray-300 group-hover:text-gray-300'
            aria-hidden='true'
...
export const Breadcrumb = memo(
  ({ items, showHome = true, maxVisible = 4, className }: BreadcrumbProps) => {
    ...
    return (
      <nav aria-label='Breadcrumb' className={className}>
        <ol aria-label='Breadcrumbs' className='relative flex gap-[12px]'>
          {/* Home item */}
          {showHome && (
