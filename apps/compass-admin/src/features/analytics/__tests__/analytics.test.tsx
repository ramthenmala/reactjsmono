...
describe('AnalyticsPage', () => {
  ...
  it('renders all analytics sections', () => {
    ...
    expect(screen.getByTestId('analytics-section-investor-insights')).toBeInTheDocument();
    expect(screen.getByTestId('analytics-section-national')).toBeInTheDocument();
    expect(screen.getByTestId('analytics-section-regional')).toBeInTheDocument();
  });

  it('passes correct props to each analytics section', () => {
    render(<AnalyticsPage />);
    
    const investorInsights = screen.getByTestId('analytics-section-investor-insights');
    expect(investorInsights).toHaveAttribute('data-section-title', 'analytics.investorInsights.title');
    expect(investorInsights).toHaveAttribute('data-is-active', 'false');
    
    const national = screen.getByTestId('analytics-section-national');
    expect(national).toHaveAttribute('data-section-title', 'analytics.national.title');
    expect(national).toHaveAttribute('data-is-active', 'false');
    
    const regional = screen.getByTestId('analytics-section-regional');
    expect(regional).toHaveAttribute('data-section-title', 'analytics.regional.title');
  ...
  it('renders sections in correct order', () => {
    ...
    expect(sections[0]).toHaveAttribute('data-qa-id', 'analytics-section-investor-insights');
    expect(sections[1]).toHaveAttribute('data-qa-id', 'analytics-section-national');
    expect(sections[2]).toHaveAttribute('data-qa-id', 'analytics-section-regional');
  });
