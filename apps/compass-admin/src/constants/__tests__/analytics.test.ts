...
describe('analytics constants', () => {
  describe('ANALYTICS_SECTIONS', () => {
    ...
    it('has investor-insights section with correct properties', () => {
      const investorInsights = ANALYTICS_SECTIONS.find(section => section.id === 'investor-insights');
      
      expect(investorInsights).toBeDefined();
      expect(investorInsights?.title).toBe('analytics.investorInsights.title');
      expect(investorInsights?.description).toBe('analytics.investorInsights.description');
    });

    it('has industrial-city-insights section with correct properties', () => {
      const industrialCityInsights = ANALYTICS_SECTIONS.find(section => section.id === 'industrial-city-insights');
      
      expect(industrialCityInsights).toBeDefined();
      expect(industrialCityInsights?.title).toBe('analytics.industrialCityInsights.title');
      expect(industrialCityInsights?.description).toBe('analytics.industrialCityInsights.description');
    });

    it('has national section with correct properties', () => {
      const national = ANALYTICS_SECTIONS.find(section => section.id === 'national');
    ...
    it('has regional section with correct properties', () => {
      const regional = ANALYTICS_SECTIONS.find(section => section.id === 'regional');
    ...
    it('has city-metrics section with correct properties', () => {
      const cityMetrics = ANALYTICS_SECTIONS.find(section => section.id === 'city-metrics');
      
      expect(cityMetrics).toBeDefined();
      expect(cityMetrics?.title).toBe('analytics.cityMetrics.title');
      expect(cityMetrics?.description).toBe('analytics.cityMetrics.description');
    });

    it('has sector-view section with correct properties', () => {
      const sectorView = ANALYTICS_SECTIONS.find(section => section.id === 'sector-view');
