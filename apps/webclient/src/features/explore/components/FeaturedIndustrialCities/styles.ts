export const featuredCitiesStyles = {
  ...
  content: {
    ...
    header: {
      ...
      subtitle: "font-medium text-base leading-[22px] tracking-normal md:text-[23px] md:leading-[30px] text-gray-600"
    }
  },
  
  mobileSlider: {
    container: "md:hidden flex items-center justify-center overflow-hidden pb-4",
    containerStyle: {
      touchAction: "pan-x" as const,
      width: "100%",
      maxWidth: "100%"
    },
    track: (isRTL: boolean) => `flex transition-transform duration-300 ease-in-out ${isRTL ? "flex-row-reverse" : ""}`,
  ...
  desktopGrid: "hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pt-8 items-start"
};