              ...
              {item.hasDropdown ? (
                <div>
                  <button 
                    ...
                    onClick={() => setMobileActiveDropdown(prev => prev === item.dropdownId ? null : (item.dropdownId || null))}
                  >
                  ...
                  {mobileActiveDropdown === item.dropdownId && item.submenu && (
                    <div className="mt-3 pl-2 space-y-4">
                      {item.submenu.sections.map((section: any, sectionIndex: number) => (
                        <div key={sectionIndex}>
                          <div className="text-sm text-white/70 mb-2">{section.title}</div>
                          <div className="grid grid-cols-2 gap-2">
                            {section.columns.flat().map((link: any, idx: number) => (
                              <Link 
                                key={idx} 
                                to={link.href} 
